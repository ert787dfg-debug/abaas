...
function setTheme(next) {
  ...
  const val = next || (curr === 'light' ? 'dark' : 'light');
  root.setAttribute('data-theme', val);
  els.theme.textContent = val === 'light' ? '🌙' : '☀️';
}
...
function renderQuestion() {
  ...
  if (state.idx < 0 || state.idx >= qs.length) {
    els.qText.textContent = state.lang === 'en'
      ? `Finished! Score: ${state.score}/${qs.length}`
      : `انتهاء! الدرجة: ${state.score}/${qs.length}`;
    els.options.innerHTML = '';
    els.feedback.hidden = true;
...
function handleAnswer(i) {
  ...
  els.feedback.textContent = isCorrect ? (fb.correct || 'Correct.') : (fb.wrong || 'Wrong.');
...
async function loadQuestions() {
  ...
  state.questions = state.questions.filter(q =>
    q.text && Array.isArray(q.choices) && typeof q.answer === 'number'
  );
}
